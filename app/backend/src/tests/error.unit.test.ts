...
describe('Error middleware', function () {
  ...
  it('error without status code return status 500', async function () {
  ...
  it('error with status code return status', async function () {
  ...
  it('returns error messsage', async function () {
    const error500 = new Error('Error without status');
    errorMiddleware(error500, req, res, next);
